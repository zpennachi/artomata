        ...
        let particleTypes;
        let particleBoundary = new THREE.Vector3(50, 50, 50);
        let maxVelocity = .1;
        let boundaryMesh;
...
let attachedParticleIndex = -1; // Initialize attachedParticleIndex variable
        ...
        function init() {
            ...
            scene = new THREE.Scene();
                    initStarfield(); // Call this in your init function
            camera = new THREE.PerspectiveCamera(35, window.innerWidth / window.innerHeight, 0.1, 6000);
  ...
  adjustParticleCount('yellow', parseInt(e.target.value));
        ...
        function animate() {
            renderer.setAnimationLoop(() => {
                ...
                updateCamera();
                controls.update(); 
                            updateStarfield(); // Function to update the starfield
                render();
            });
        }  
...
function initStarfield() {
    ...
    for (let i = 0; i < starCount; i++) {
        ...
        lerpFactors.push(Math.random() * 0.15); // Lerp factor between 0 and 0.05
    }

    starsGeometry.setAttribute('position', new THREE.BufferAttribute(positions, 3));
    const starsMaterial = new THREE.PointsMaterial({ color: 0x000000, size: 0.1, transparent: true });

    const starField = new THREE.Points(starsGeometry, starsMaterial);
    starField.name = 'starField';
    starField.userData.initialPositions = initialPositions;
    starField.userData.targetPositions = targetPositions;
    starField.userData.lerpFactors = lerpFactors;
    scene.add(starField);
    return starField;
}
...
function updateStarfield() {
    ...
    if (!starField || !starField.userData || !starField.userData.initialPositions) {
        console.error('Starfield or initial positions not found!');
        return; // Exit if not found or not set
    }

    const positions = starField.geometry.attributes.position;
    const initialPositions = starField.userData.initialPositions;
    const targetPositions = starField.userData.targetPositions;
    const lerpFactors = starField.userData.lerpFactors;

    if (!positions || !initialPositions || !targetPositions || !lerpFactors) {
        console.error('Missing position data or arrays not initialized!');
        return; // Ensure all arrays are correctly referenced
    }

    for (let i = 0; i < positions.count; i++) {
        const index = i * 3;
        if (positions.array.length < index + 3) continue; // Safety check for array bounds
        ...
        // Ensure targetPositions[i] is defined
        if (!targetPositions[i]) continue; // Skip if undefined

        position.lerp(targetPositions[i], lerpFactors[i]);
        positions.array[index] = position.x;
        positions.array[index + 1] = position.y;
        positions.array[index + 2] = position.z;

        // Randomly decide to update target position
        if (Math.random() < 0.01) {
            targetPositions[i].set(
                initialPositions[i].x + (Math.random() - 0.5) * 2,
                initialPositions[i].y + (Math.random() - 0.5) * 2,
                initialPositions[i].z + (Math.random() - 0.5) * 2
            );
        }
    }
  ...
  function updateCamera() {
    if (isAttachedToParticle) {
        ...
        // Smoothly interpolate camera position and target towards the target position
...
function updateParticles() {
        ...
        for (let i = 0; i < type.instances; i++) {
            ...
            ['x', 'y', 'z'].forEach(axis => {
                if (position[axis] > particleBoundary[axis]) {
                    position[axis] = -particleBoundary[axis]; // Wrap to the opposite side
            ...
            // Update dummy object position and scale
            dummy.position.copy(position);
    ...
    const speed = 10.1; // Adjust as needed
    const moveDistance = speed; // Adjust as needed
    const moveVector = new THREE.Vector3();
